...
void caffe_test_dumpBuffer_inner_product(void* buf, size_t numElements, std::string layername, std::string path)
{
    ...
    {
        printf("CAFFE INNER_PRODUCT WRITE: Writing file %s into caffe_local_output folder\n", fileName.c_str());
...
//if(this->layer_param().name() == "fc6")
  caffe_test_dumpBuffer_inner_product(top[0]->mutable_cpu_data(), top[0]->count(), this->layer_param().name(), "caffe_local_output/");
#endif
}
