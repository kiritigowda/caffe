...
void caffe_test_dumpBuffer_softmax(void* buf, size_t numElements, std::string layername, std::string path)
{
    ...
    {
        printf("CAFFE SOFTMAX WRITE: Writing file %s into caffeBufferDump folder\n", fileName.c_str());
...
//if(this->layer_param().name() == "prob")
  caffe_test_dumpBuffer_softmax(top[0]->mutable_cpu_data(), top[0]->count(), this->layer_param().name(), "caffeBufferDump/");
#endif
}
