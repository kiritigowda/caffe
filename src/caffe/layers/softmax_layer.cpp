...
void caffe_test_dumpBuffer_softmax(void* buf, size_t numElements, std::string layername, std::string path)
{
    ...
    printf("CAFFE SOFTMAX WRITE: Writing file %s with %d elements\n", fileName.c_str(), (int)numElements);

    FILE * fp = fopen(fileName.c_str(), "wb");
    if(!fp) printf("Could not open file %s\n", fileName.c_str());
    else
    {
        printf("CAFFE SOFTMAX WRITE: Writing file %s into caffe_local_output folder\n", fileName.c_str());
...
#endif
//if(this->layer_param().name() == "prob")
  caffe_test_dumpBuffer_softmax(top[0]->mutable_cpu_data(), top[0]->count(), this->layer_param().name(), "caffe_local_output/");
#endif
}
